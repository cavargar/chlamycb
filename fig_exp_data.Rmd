...
pf_7p<-function(ss,sig,zf){
  p<-mapply(p_fp,expmoddf$vol,expmoddf$ndiv,ss,sig,zf[1],zf[2],zf[3],zf[4],zf[5])
  return(p)
}
```
...
expmoddf$p_7p<-pf_7p(141.78, 0.31, c(0.73,1.01,2.16,3.03,4))
...
gpexpmod0<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==0))+
  ...
  theme_classic()
gpexpmod0
ggsave("wt_prob_mth_div0.png",width = 15,height = 3)
```
```{r}
expmoddf%>%
