...
```{r}
exp_data <- read_excel("mat3_small.xlsx")
exp_data
```
...
gpdata<-ggplot(exp_data,aes(x=vol,y=as.factor(ndiv),color=as.factor(ndiv)))+geom_jitter()+scale_x_log10(limits=c(10,3000))+ theme(axis.title.x = element_blank())+ylab("Divisions")
#gpdata
...
gpdens<-ggplot(scatp,aes(x=ms,y=pscat,color=as.factor(ndiv)))+geom_point(size=2)+geom_line(size=1)+scale_x_log10(limits=c(10,3000)) + theme(axis.title.x = element_blank())+ylab("Probability")
...
expmoddf$p_pl<-pf_pl(62.78,0.60)
expmoddf$p_3p<-pf_4p(70.49,0.57,c(0,0.5))
expmoddf$p_4p<-pf_4p(66.49,0.53,c(0.17,0.61))
...
gpdata<-ggplot(exp_data,aes(x=vol,y=as.factor(ndiv),color=as.factor(ndiv)))+
  ...
  theme_classic()
gpdata
ggsave("mat3_div_mth.png",width = 15,height = 3)
```
```{r fig.height=3, fig.width=15}
gpdens<-gpdens<-ggplot(scatp,aes(x=ms,y=pscat,color=as.factor(ndiv)))+
  ...
  theme_classic()
gpdens
ggsave("mat3_prob_mth.png",width = 15,height = 3)
```
```{r fig.height=3, fig.width=15}
...
gpexpmod0<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==0))+
  ...
  theme_classic()
gpexpmod0
ggsave("mat3_prob_mth_div0.png",width = 15,height = 3)
```
```{r fig.height=3, fig.width=15}
gpexpmod1<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==1))+
  ...
  theme_classic()
gpexpmod1
ggsave("mat3_prob_mth_div1.png",width = 15,height = 3)
```
```{r fig.height=3, fig.width=15}
gpexpmod2<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==2))+
  ...
  theme_classic()
gpexpmod2
ggsave("mat3_prob_mth_div2.png",width = 15,height = 3)
```
```{r fig.height=3, fig.width=15}
gpexpmod3<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==3))+
  ...
  theme_classic()
gpexpmod3
ggsave("mat3_prob_mth_div3.png",width = 15,height = 3)
```
...
gpexpmod4<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==4))+
  ...
  theme_classic()
gpexpmod4
ggsave("mat3_prob_mth_div4.png",width = 15,height = 3)
```
...
gpexpmod5<-ggplot(expmoddf%>%filter(!src=="exp",ndiv==5))+
  ...
  theme_classic()
gpexpmod5
ggsave("mat3_prob_mth_div5.png",width = 15,height = 3)
```